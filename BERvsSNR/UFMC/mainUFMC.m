...
if flagBEP_Simulation
    ...
    for rep=1:MonteCarloRepetitions
        ...
        %OFDM modulation
        s=UFMCModulation(str,LTEgrid)*NormFactor;
        %Channel model
        ch=channel();
        %Power Delay Profile
        ...
        for k=1:length(snr_dB)
            ...
            r=r_no+n;
            %Demodulation
            rx_LTEgrid=UFMCDemodulation(str,r)/NormFactor;
    ...
    if saveFlag
        if strcmp(channelmodel,'Nakagami')
        FileName = ['.\Results\' 'UFMC_Simulation_' channelmodel '_ShapeParameter' num2str(mu) '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'meanBER');
        else
        FileName = ['.\Results\' 'UFMC_Simulation_' channelmodel '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'meanBER');
        end
...
if flagBEP_Calculation
    ...
    if saveFlag
        if strcmp(channelmodel,'Nakagami')
        FileName = ['.\Results\' 'UFMC_Calculation_' channelmodel '_ShapeParameter' num2str(mu) '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'BEP');
        else
        FileName = ['.\Results\' 'UFMC_Calculation_' channelmodel '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'BEP');
        end
...
if flagPSD
    ...
    saveFlag=1;
    if saveFlag
        FileName = ['.\Results\' 'UFMC_Spectrum.mat'];
        save(FileName,...
