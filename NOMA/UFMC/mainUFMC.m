...
for k=1:length(snr_dB)
    ...
    for rep=1:MonteCarlo
        ...
        %Symbol stream
        symbols1=PowerLevel1*Bit_to_Symbol(symmap,stream1,Mod);
        symbols2=PowerLevel2*Bit_to_Symbol(symmap,stream2,Mod);
        symbols3=PowerLevel3*Bit_to_Symbol(symmap,stream3,Mod);
        ...
        %UFMC modulation
        s1=UFMCModulation(str,LTEgrid1)*NormFactor;
        s2=UFMCModulation(str,LTEgrid2)*NormFactor;
        s3=UFMCModulation(str,LTEgrid3)*NormFactor.*exp(1i*2*pi*str.Subcarrier_spacing*Spacing*t);
        s=s1+s2+s3; 
    ...
    X=1:length(PSD1);
    %X=1144:1316;       %Indices of samples representing the frequency channel
    X1=1316:1488;
    P_int=trapz(X1,PSD3(1316:1488));
    P1=trapz(X,PSD1(X));
    P2=trapz(X,PSD2(X));
    P3=trapz(X,PSD3(X));
    
    frequency_UFMC = (3.60008e9*str.NSamples *str.Sampling_time-str.NSamples/2:3.60008e9*str.NSamples *str.Sampling_time+str.NSamples/2-1)/(str.NSamples *str.Sampling_time);
    %Signal Bandwidth Computation
    b1=powerbw(PSD1, frequency_UFMC);
    b2=powerbw(PSD2, frequency_UFMC);
    b3=powerbw(PSD3, frequency_UFMC);
...
saveFlag=0;
if saveFlag
    FileName = ['.\Results\UFMC_NOMA_low.mat'];
    save(FileName,...
          'snr_dB', ...
          'c1NOMA', ...
          'c1OMA');
...
if flagPSD
    figure,
    plot( frequency_UFMC /10e8 , 10*log10(PSD1) ,'b')
    %powerbw(PSD1, frequency_UFMC);
    hold on
    plot( frequency_UFMC/10e8  , 10*log10(PSD2) ,'r');
    plot( frequency_UFMC/10e8  , 10*log10(PSD3) ,'k');
    title('NOMA with UFMC waveforms');
    xlabel('Frequency [GHz]');
    ylabel('PSD [dB/Hz]');
    legend('Signal 1', 'Signal 2', 'Signal 3');
    axis([frequency_UFMC(1)/10e8 frequency_UFMC(length(frequency_UFMC/10e8))/10e8 -100 0]);
end
