...
if flagBEP_Simulation
    ...
    Pn_vector = 10.^(-snr_dB/10)*2;   %Noise vector 
    MonteCarloRepetitions=8000;
    for rep=1:MonteCarloRepetitions
        ...
        %OFDM modulation
        s=FBMCModulation(str,LTEgrid)*NormFactor;
        %Channel model
        ch=channel();
        %Power Delay Profile
        ...
        for k=1:length(snr_dB)
            ...
            r=r_no+n;
            %Demodulation
            rx_LTEgrid=FBMCDemodulation(str,r)/NormFactor;
    ...
    if saveFlag
        if strcmp(channelmodel,'Nakagami')
        FileName = ['.\Results\' 'FBMC_Simulation_' channelmodel '_ShapeParameter' num2str(mu) '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod1) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'meanBER');
        else
        FileName = ['.\Results\' 'ProvaFBMC_Simulation_' channelmodel '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod1) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'meanBER');
        end
...
if flagBEP_Calculation
    %Modulation order
    mod=4;
    [symmap,bitmap]=ConstellationPAM(sqrt(mod));
    %FBMC object creation
    str=FBMC();   
    NSamples=str.NSamples;
    %SNR and simulated noise power
    snr_dB=-6:2:30;
    Pn_vector = 10.^(-snr_dB/10)*2;
    %Matrices
    G=TXMatrix(str);    %TX Matrix                                                        %,ResourceGrid
    Q=RXMatrix(str,G);  %RX Matrix
    Q=Q';
    ...
    for m = 1:length(Pn_vector)
        ...
        for l = 1:size(symmap,1)
            ...
            for k = 1:NrMonteCarloRepetitionsForInterferers
                ...
                x = symmap(randi(sqrt(mod),str.Subcarriers*str.Symbols,1));
                x(MidPosition) = xlk;  
    ...
    if saveFlag
        if strcmp(channelmodel,'Nakagami')
        FileName = ['.\Results\' 'FBMC_Calculation_' channelmodel '_ShapeParameter' num2str(mu) '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'BEP');
        else
        FileName = ['.\Results\' 'FBMC_Calculation_' channelmodel '_Velocity' int2str(ch.Velocity_kmh) '_Modulation' int2str(mod) 'QAM.mat'];
        save(FileName,...
              'snr_dB', ...
              'BEP');
        end
...
if flagPSD
    ...
    saveFlag=1;
    if saveFlag
        FileName = ['.\Results\' 'FBMC_Spectrum.mat'];
        save(FileName,...
