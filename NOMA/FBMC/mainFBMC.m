...
Mod1=16;
Mod=sqrt(Mod1);
[symmap,bitmap]=ConstellationPAM(Mod);
%FBMC object creation
str=FBMC();   
...
for k=1:length(snr_dB)
    ...
    for rep=1:MonteCarlo
        ...
        %FBMC modulation
        s1=FBMCModulation(str,LTEgrid1)*NormFactor;
        s2=FBMCModulation(str,LTEgrid2)*NormFactor;
        s3=FBMCModulation(str,LTEgrid3)*NormFactor.*exp(1i*2*pi*str.Subcarrier_spacing*Spacing*t);
        s=s1+s2+s3; 
    ...
    frequency_FBMC = (3.60008e9*str.NSamples *str.Sampling_time-str.NSamples/2:3.60008e9*str.NSamples *str.Sampling_time+str.NSamples/2-1)/(str.NSamples *str.Sampling_time); 
    %Signal Bandwidth Computation
    b1=powerbw(PSD1, frequency_FBMC);
    b2=powerbw(PSD2, frequency_FBMC);
    b3=powerbw(PSD3, frequency_FBMC);
...
saveFlag=0;
if saveFlag
    FileName = ['.\Results\FBMC_NOMA_low.mat'];
    save(FileName,...
          'snr_dB', ...
          'c1NOMA', ...
          'c1OMA');
...
if flagPSD
    figure,
    plot( frequency_FBMC /10e8 , 10*log10(PSD1) ,'b')
    %powerbw(PSD1, frequency_OFDM);
    hold on
    plot( frequency_FBMC/10e8  , 10*log10(PSD2) ,'r');
    plot( frequency_FBMC/10e8  , 10*log10(PSD3) ,'k');
    title('NOMA with FBMC waveforms');
    xlabel('Frequency [GHz]');
    ylabel('PSD [dB/Hz]');
    legend('Signal 1', 'Signal 2', 'Signal 3');
    axis([frequency_FBMC(1)/10e8 frequency_FBMC(length(frequency_FBMC/10e8))/10e8 -100 0]);
end
